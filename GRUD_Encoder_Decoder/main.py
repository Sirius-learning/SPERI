...
for i in range(0,7):
    ...
    pureGRUD_result_record_path = os.path.join(current_path,"GRUD_results_score.txt")
    ...
    cst_masked_normalized_data_path = os.path.join(current_path, "data", "SDU_PVdata_masked_consective_" + str(int(miss_rate * 100)) + "%.csv")
    point_masked_normalized_data_path = os.path.join(current_path, "data", "SDU_PVdata_masked_" + str(int(miss_rate * 100)) + "%.csv")
    ...
    pvdata = [pv_xzero, pv_m, pv_d, pv_xpie, pv_label]

    #读取没被stl分解的数据
    row_x = pd.read_csv(point_masked_normalized_data_path)
    #row_x = pd.read_csv(point_masked_normalized_data_path)
    colnum = int(row_x.shape[0]/96)
    row_x = row_x['realPower'].values.reshape(colnum,96)
    ...
    train_dataset = [x_train, m_train, d_train, xpie_train, label_train]
    dev_dataset = [x_dev, m_dev, d_dev, xpie_dev, label_dev]
    test_dataset = [x_test, m_test, d_test, xpie_test, label_test]
    ...
    #record result
    file = open(pureGRUD_result_record_path, 'a')
    file.write('GRUD_encoder_decoder_traindata10%,point miss,miss rate:{}\n'.format(str(int(miss_rate * 100))))
